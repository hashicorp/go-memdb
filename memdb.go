...
type MemDB struct {
	schema  *DBSchema
	root    unsafe.Pointer // *iradix.Tree[*iradix.Tree[any]] underneath
	primary bool
...
func NewMemDB(schema *DBSchema) (*MemDB, error) {
	...
	db := &MemDB{
		schema:  schema,
		root:    unsafe.Pointer(iradix.New[*iradix.Tree[any]]()),
		primary: true,
	}
...
func (db *MemDB) getRoot() *iradix.Tree[*iradix.Tree[any]] {
	root := (*iradix.Tree[*iradix.Tree[any]])(atomic.LoadPointer(&db.root))
	return root
}
