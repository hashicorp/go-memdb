...
func TestTxn_IsolationInt(t *testing.T) {
	db := testDB(t)
	txn1 := db.Txn(true)
...
func TestTxn_AbortInt(t *testing.T) {
	db := testDB(t)
	txn1 := db.Txn(true)
...
func TestComplexIntDBInt(t *testing.T) {
	db := testComplexDBInt(t)
	testPopulateDataInt(t, db)
	...
	raw, err = txn.First("people", "id_prefix", raw.(*TestPersonInt).ID[:4])
	noErr(t, err)
	if raw == nil {
		t.Fatalf("should get person")
	}
	...
	if raw.(*TestPersonInt).First != "Mitchell" {
		t.Fatalf("wrong person!")
	}

	raw, err = txn.First("people", "age", int64(23))
	noErr(t, err)
	if raw == nil {
		t.Fatalf("should get person")
	}

	person := raw.(*TestPersonInt)
	...
	place := raw.(*TestPlaceInt)
	if place.Name != "Maui" {
		t.Fatalf("bad place (but isn't anywhere else really?): %v", place)
	}
}

func TestWatchUpdateInt(t *testing.T) {
	db := testComplexDBInt(t)
	testPopulateDataInt(t, db)
	txn := db.Txn(false) // read only
...
func testPopulateDataInt(t *testing.T, db *MemDB) {
	// Start write txn
	txn := db.Txn(true)

	// Create some data
	person1 := testPersonInt()

	person2 := testPersonInt()
	person2.First = "Mitchell"
	person2.Last = "Hashimoto"
	person2.Age = 27

	person3 := testPersonInt()
	person3.First = "Alex"
	person3.Last = "Dadgar"
	person3.Age = 23
	...
	visit1 := &TestVisitInt{person1.ID, place1.ID}
	visit2 := &TestVisitInt{person2.ID, place2.ID}
...
type TestPersonInt struct {
	ID      string
	First   string
	Last    string
	Age     int64
	Sibling *TestPersonInt
}

type TestPlaceInt struct {
	ID   string
	Name string
}

type TestVisitInt struct {
	Person string
	Place  string
}

func testComplexSchemaInt() *DBSchema {
	return &DBSchema{
			...
			"people": &TableSchema{
				...
				Indexes: map[string]*IndexSchema{
					...
					"age": &IndexSchema{
						Name:    "age",
						Unique:  false,
						Indexer: &IntFieldIndex{Field: "Age"},
					},
					"sibling": &IndexSchema{
						Name:    "sibling",
						Unique:  false,
...
func testComplexDBInt(t *testing.T) *MemDB {
	db, err := NewMemDB(testComplexSchemaInt())
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	return db
...
func testPersonInt() *TestPersonInt {
	_, uuid := generateUUID()
	obj := &TestPersonInt{
		ID:    uuid,
		First: "Armon",
...
func testPlaceInt() *TestPlaceInt {
	_, uuid := generateUUID()
	obj := &TestPlaceInt{
		ID:   uuid,
		Name: "HashiCorp",
